...
# 1. Update Environment Variables and DSN for MySQL
...
{
    # 2. Check for MySQL driver instead of Oracle
    ...
    if ($@) {
        plan skip_all => "Cannot connect to MySQL ($DSN): $@";
    }
}

# 3. Simplify Drop Table logic (MySQL supports IF EXISTS)
sub drop_tables {
    my ($dbh) = @_;
    # Disable foreign key checks to drop in any order
...
# 4. Convert Oracle Types to MySQL Types[cite: 1]
$raw_dbh->do(q{
    CREATE TABLE users (
